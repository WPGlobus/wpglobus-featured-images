	...
	class WPGlobus_Featured_Images {
				...
				case 'wpglobus-remove-post-thumbnail':
					...
					if ( $order['language'] == WPGlobus::Config()->default_language ) {
						...
						if ( delete_post_meta( $order['attr']['post_id'], '_thumbnail_id' ) ) {
							...
							$ajax_return['html'] 	= $this->_post_thumbnail_html( null, get_post($order['attr']['post_id']), $order['language']  );
						} else {
						...
						$wpglobus_thumbnail_ids = (array) get_post_meta( $order['attr']['post_id'], 'wpglobus_thumbnail_ids', true );
				...
				case 'wpglobus-set-post-thumbnail':
					...
					if ( $order['language'] == WPGlobus::Config()->default_language ) {
						...
						if ( update_post_meta( $order['attr']['post_id'], '_thumbnail_id', $order['attr']['thumbnail_id'] ) ) {
							$ajax_return['result']  = 'ok';
							$ajax_return['html'] 	= $this->_post_thumbnail_html( $order['attr']['thumbnail_id'], get_post($order['attr']['post_id']), $order['language']  );
						} else {
		...
		function on_post_thumbnail_html( $html, $post_id, $post_thumbnail_id, $size, $attr ) {
			...
			if ( WPGlobus::Config()->language != WPGlobus::Config()->default_language ) {
				...
				if ( !empty($wpglobus_thumbnail_ids[WPGlobus::Config()->language]) ) {
					$html = wp_get_attachment_image( $wpglobus_thumbnail_ids[WPGlobus::Config()->language], $size, false, $attr );
				}
			}	
		...
		function featured_images_tabs( $thumbnail_id_default_language = null, $wpglobus_thumbnail_ids = null, $post = null ) {
				...
				<div id="wpglobus-featured-images-tabs" class="wpglobus-post-body-tabs" data-featured-image-language="">
					...
					foreach ( WPGlobus::Config()->open_languages as $language ) {	?>
						<div id="featured-images-tab-<?php echo $language; ?>" class="wpglobus-featured-images-general" 
							data-language="<?php echo $language; ?>">
							<?php 
		...
		function _post_thumbnail_html( $thumbnail_id = null, $post = null, $language ) {
			...
			$set_thumbnail_link = '<p style="clear:both;" class="hide-if-no-js"><a title="' . esc_attr__( 'Set featured image' ) . '" href="%s" id="set-post-thumbnail-' . $language . '" class="thickbox wpglobus-set-post-thumbnail" data-language="'.$language.'">%s</a></p>';
			...
			if ( $thumbnail_id && get_post( $thumbnail_id ) ) {
				...
				if ( !empty( $thumbnail_html ) ) {
					...
					$onclick = "WPGlobusFeaturedImages.removeThumbnail('" . $ajax_nonce . "', '" . $language . "', '" . $post->ID . "');return false;";
